...
if os.path.normcase(project_root) not in normalized_sys_path:
    sys.path.insert(0, project_root)
normalized_sys_path = [os.path.normcase(os.path.abspath(p)) for p in sys.path]
if os.path.normcase(backend_path) not in normalized_sys_path:
...
def register_technical_debt(pattern: str, notes: str, line_number: Optional[int] = None, exception: Optional[Exception] = None):
    ...
    try:
        ...
        store.register_debt(
            ...
            file_path="scratch/dispatch_next_batch.py",
    ...
    except (ValueError, OSError, KeyError, TypeError, json.JSONDecodeError) as register_err:
...
def handle_hub_exceptions(pattern: str, error_prefix: str):
    ...
    try:
        ...
        traceback.print_exc(file=sys.stderr)
        raise
    except (RuntimeError, IndexError, NameError, AssertionError) as e:
...
def run_dispatch(args=None) -> Optional[Dict[str, Any]]:
    ...
    if parsed_args.phase is not None:
        if isinstance(parsed_args.phase, bool) or not isinstance(parsed_args.phase, int) or parsed_args.phase <= 0:
    ...
    if parsed_args.batch_size is not None:
        if isinstance(parsed_args.batch_size, bool) or not isinstance(parsed_args.batch_size, int) or parsed_args.batch_size <= 0:
