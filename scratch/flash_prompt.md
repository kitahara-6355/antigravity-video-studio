...
- **モード**: STANDARD（平日日中）
- **MAX_PARALLEL (batch_size)**: 6件
- **タイマー間隔**: 300秒
- **アーカイブ閾値**: 15バッチ / 80タスク / 3時間
- **ステータス出力**: full
- **サブエージェントTMO**: 600秒
- **バッチTMO**: 900秒
- **コンテキスト推定**: 6%/バッチ

**MAX_PARALLEL の適用**: `get_next_batch()` 呼出時に `batch_size=6` を使用すること。
...
### Opus Directive（戦略指示）
- **Directive ID**: `D-opus-session-p27-diversified-v1`
- **優先度配分**: `thumbnail`: 30%, `test_weaver`: 25%, `bug_hunter`: 20%, `refactor`: 15%, `tdr_cleanup`: 10%
- **重点モジュール**: `subtitle_normalizer.py`, `youtuber_grade_scorer.py`, `preview_report_generator.py`, `themes_router.py`, `orchestrator.py`
- **戦略メモ**: Phase 27準備: エージェント偏り是正（thumbnail 100%→30%）。test_weaver/bug_hunter/refactor/tdr_cleanupを活性化し、NHK字幕スコアラ・YouTuber CTRベンチマーク関連モジュールを重点対象に設定。

### バッチ状態
- **現在バッチID**: `batch_3f4c3a`
- **タスク状態**: pending=0, running=0, pass=6, fail=0
- **サムネイルタスク状態**: 合計=2件 (pending=0, running=0, pass=2, fail=0)

### セッション履歴
前回セッション実績: 112タスク完了 / 19バッチ処理

### ユーザー着席スケジュール
- **本日**: 火曜日 — 平日（定期チェックモード）
- **着席窓**: 06:30-07:10 (朝チェック) / 08:50-09:00 (午前短窓) / 12:00-12:30 (昼チェック) / 15:00-15:15 (午後短窓) / 17:50-18:00 (夕方短窓) / 21:30-22:00 (夜チェック)
- **遵守率**: 70%（定期6回 + 任意3回/日）
