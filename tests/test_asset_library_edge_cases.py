...
class TestAssetLibraryEdgeCases:
    ...
    def test_additional_hex_to_color_name_invalid_types(self):
        """_hex_to_color_name に不正な型（None, int, list等）を渡した際に AttributeError などが起きることを検証"""
    ...
    def test_additional_hex_to_color_name_giant_input(self):
        """_hex_to_color_name に巨大な入力（数万文字）を渡した場合の検証"""
        giant_hex = "A" * 100000
        # startswith 等で引っかからず、明度判定でエラーになり "" が返ることを確認
        assert _hex_to_color_name(giant_hex) == ""

    def test_additional_get_assets_for_task_invalid_types_and_empty(self, mock_env):
        """get_assets_for_task に None や空リストなどの不正な型を渡したときの挙動"""
        library = CreativeAssetLibrary(asset_root=mock_env)
        # 空文字列タスク
        res_empty = library.get_assets_for_task("")
        assert res_empty["available"] == []
        assert res_empty["recommended"] == []
        assert res_empty["missing"] == []

        # 不正な型（None）-> requirements.get(task_type) は None になり、required_types = [] になるため安全に動作する
        res_none = library.get_assets_for_task(None)
        assert res_none["available"] == []
        assert res_none["recommended"] == []
        assert res_none["missing"] == []

    def test_additional_get_usage_report_empty_and_giant_and_none(self, mock_env):
        """get_usage_report に対する None, 空リスト, 巨大入力テスト"""
        library = CreativeAssetLibrary(asset_root=mock_env)
        
        # 1. 空リスト
        res_empty = library.get_usage_report([])
        assert res_empty["total_referenced"] == 0
        assert res_empty["referenced_assets"] == []

        # 2. None入力 -> イテレート不可で TypeError になることを確認
        with pytest.raises(TypeError):
            library.get_usage_report(None)

        # 3. 巨大入力 (存在しないアセットIDの大量リスト)
        giant_list = [f"non_existent_{i}" for i in range(10000)]
        res_giant = library.get_usage_report(giant_list)
        assert res_giant["total_referenced"] == 0
        assert res_giant["referenced_assets"] == []

    def test_additional_dataclasses_invalid_types(self):
        """AssetEntry や GuestProfile に対して不正な型やNoneを渡して初期化した際の型挙動検証"""
        entry = AssetEntry(
            id=None,
            path=123,
            filename=None,
    ...
    def test_additional_load_index_malformed_assets(self, mock_env):
        """インデックス内のアセットエントリに必要なキーが不足していてTypeErrorが起きる場合、安全に無視・再作成されることを検証"""
        index_path = mock_env / "asset_index.json"
        # 'id' などの必須フィールドが欠損しているアセットエントリのリスト
        malformed_data = {
            "version": "2.0",
            "assets": [
                {"path": "brand/music/bgm.mp3", "filename": "bgm.mp3"}  # id, type, category が欠損
            ]
        }
        ...
        library = CreativeAssetLibrary(asset_root=mock_env)
        # TypeErrorが発生するはずなので、インデックスは読み込みエラーになり、assetsが空になることを確認
        assert library.assets == []

    def test_additional_search_assets_extreme_query(self, mock_env):
        """極端に長いクエリやNoneを検索クエリに指定した場合の安全性の検証"""
        library = CreativeAssetLibrary(asset_root=mock_env)
        mock_engine = MagicMock()
        mock_engine.get_index_stats.return_value = {"total_entries": 1}
        mock_engine.search.return_value = []
        
        with patch("services.vector_search.vector_search_engine", mock_engine):
            # 1. 巨大クエリ
            giant_query = "a" * 50000
            res_giant = library.search_assets(giant_query)
            assert res_giant == []
            mock_engine.search.assert_called_with(query=giant_query, top_k=5)
            
            # 2. Noneクエリ (L529のログ出力でquery[:30]が呼ばれるためTypeErrorが発生することを検証)
            with pytest.raises(TypeError):
                library.search_assets(None)

    def test_additional_get_usage_report_duplicate_ids(self, mock_env):
        """get_usage_report に重複したアセットIDが含まれる場合の挙動"""
        library = CreativeAssetLibrary(asset_root=mock_env)
        asset = AssetEntry(
            id="a001", path="photos/p1.jpg", filename="p1.jpg",
            type="photo", category="channel_owner"
        ...
        # 同じ ID を 3 回指定してレポートを取得
        report = library.get_usage_report(["a001", "a001", "a001"])
        assert report["total_referenced"] == 3
        # usage_count が 3 にインクリメントされること
        assert library.assets[0].usage_count == 3
