...
def test_transcribe_video_sync_restores_event_loop(mock_whisper_transcriber):
    """イベントループが実行前後に正しく退避・復元されることを検証"""
    import asyncio
...
def test_cli_import_and_execution_via_subprocess():
    """CLIとして外部プロセスから呼び出した場合、インポートエラーにならず引数エラー(終了コード1)になることを検証"""
    import subprocess
    import sys
    
    # ルートディレクトリの transcribe_sync.py を呼び出す
    script_path = str(Path(__file__).parent.parent / "transcribe_sync.py")
    
    # 引数なしで実行して終了コードが 1 になること（ModuleNotFoundError にならずに sys.exit(1) に到達すること）
    ...
    assert "使用方法:" in result.stdout or "使用方法:" in result.stderr
