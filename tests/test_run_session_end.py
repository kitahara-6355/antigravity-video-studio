...
@pytest.mark.xfail(reason="sys.modules cache reload side-effects in full test suites")
...
# --- Phase 33 バグ修正に伴う新規テスト ---

def test_verify_session_state_none_values():
    """verify_session_state において、必須フィールドの値が None の場合に TypeError が発生することを検証"""
    from backend.agents.orchestration.run_session_end import SessionEndManager
    from unittest.mock import MagicMock
    manager = SessionEndManager(hub=MagicMock())
    
    # tasks_completed_in_session が None の場合
    invalid_session = {
        "tasks_completed_in_session": None,
        "batches_in_session": 5,
        "context_consumption_pct": 50,
    ...
    # current_batch_id が None の場合
    invalid_session_batch = {
        "tasks_completed_in_session": 10,
        "batches_in_session": 5,
        "context_consumption_pct": 50,
        "current_batch_id": None
    ...
    with pytest.raises(TypeError) as exc_info:
        manager.verify_session_state(invalid_session_batch)
    assert "str である必要があります" in str(exc_info.value)


def test_traceback_deprecation_warning_free(capsys):
    """log_error でエラーを出力した際、traceback.print_exception の呼び出しで DeprecationWarning が発生しないことを検証"""
    import warnings
    from backend.agents.orchestration.run_session_end import SessionEndErrorHandler
    ...
    ctx = {
        ...
        "phase": 33,
        "milestone": "M33.1",
        "timestamp": "20260613_000000_UTC",
        "extra_info": None
    }
    
    # DeprecationWarning をエラーとして扱い、警告が発生すれば例外になるようにする
    with warnings.catch_warnings():
        warnings.simplefilter("error", DeprecationWarning)
        try:
            handler.log_error(Exception("Test Deprecation Warning"), ctx)
        except DeprecationWarning as w:
            pytest.fail(f"DeprecationWarning raised by traceback.print_exception: {w}")
        
    captured = capsys.readouterr()
    assert "Warning: Unexpected error during phase state retrieval: Test Deprecation Warning (Exception)" in captured.err


@patch("backend.agents.memory.technical_debt.TechnicalDebtStore")
def test_register_debt_with_invalid_context_type_caught(mock_store_class, capsys):
    """contextが不正な型（dictではないなど）の場合でも、register_debt_if_neededでAttributeErrorが適切に処理されることを検証"""
    import os
    from backend.agents.orchestration.run_session_end import SessionEndErrorHandler
    ...
    try:
        ...
        # context として型ヒントを無視して文字列を渡す
        invalid_context = "not_a_dict"
        
        # 期待されない例外を渡し、技術負債登録で AttributeError が誘発されるようにする
        handler.register_debt_if_needed(NotImplementedError("unexpected"), invalid_context) # type: ignore
        
        captured = capsys.readouterr()
        # AttributeError が catch されて warning が出力されること
        assert "Warning: Failed to register technical debt:" in captured.err
        mock_store.register_debt.assert_not_called()
    finally:
        os.environ.pop("FORCE_DEBT_REGISTRATION", None)
