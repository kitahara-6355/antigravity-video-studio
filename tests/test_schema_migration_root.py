...
def test_main_block(monkeypatch, tmp_path):
    ...
    import backend.ux_verification.schema_migration as sm
    schema_migration_path = sm.__file__

    monkeypatch.setattr(sys, "argv", ["schema_migration.py"])
    res = runpy.run_path(schema_migration_path, run_name="__main__")
    assert res is not None

    monkeypatch.setattr(sys, "argv", ["schema_migration.py", "--apply"])
    res_apply = runpy.run_path(schema_migration_path, run_name="__main__")
...
def test_validate_v2_schema_extra_edge_cases():
    ...
    errors = validate_v2_schema(story_invalid_layer_type)
    ...
    errors = validate_v2_schema(story_invalid_layer_float)
    assert any("layer が 1-5 の範囲外です" in err for err in errors)
