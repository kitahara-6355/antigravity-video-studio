...
async def test_thumbnail_generator_fallback_on_api_failure(tmp_path):
    ...
    with patch.object(generator.client.models, "generate_images", side_effect=errors.APIError(500, "API Error")), \
         patch.object(generator, "_get_brand_style", return_value="Test brand style"), \
...
async def test_thumbnail_generator_api_error_concept():
    ...
    with patch.object(generator.client.models, "generate_content", side_effect=errors.APIError(500, "API Error")):
...
@pytest.mark.asyncio
async def test_phase27_thumbnail_quality_compliance(tmp_path):
    """
    Phase 27 thumbnail タスク #1 の品質基準を検証するテスト:
    ...
    - デフォルトフォントかつ日本語タイトルでの 'PREMIUM PREVIEW' への置換が機能していること
    """
    db_file = tmp_path / "phase27_thumbnail.db"
    task_id = "phase27_task_001"
    
    # 1. 改善されたフォールバック画像生成プレビューロジックの検証 (デフォルトフォント & 日本語タイトル)
    generator = ThumbnailGenerator()
    import sys
    import io
    # フォント読み込みで例外を起こしてデフォルトフォントを選択させる
    with patch("PIL.ImageFont.truetype", side_effect=Exception("Font not found")):
        # 破損画像(b"invalid")を入れて日本語タイトルを渡すことで、改善した置換ロジックが走りフォールバック画像が生成されることを確認
        img_bytes = generator.verify_and_optimize_image(b"invalid", title="日本語の動画タイトル")
    ...
    # 2. StageBoundAgent 連携および自動リトライ、結果保存、DBマイグレーションの検証
    import base64
    agent = StageBoundAgent(stage_name="thumbnail", db_path=str(db_file))
    agent.video_title = "Phase 27 Test Title"
    agent.video_description = "Phase 27 Description"
    agent.db_path = str(db_file)
    agent.output_dir = str(tmp_path)
    ...
    with patch.object(ThumbnailGenerator, "generate", return_value=mock_results):
        ...
        assert size_bytes < 4 * 1024 * 1024 # 4MB 未満
        
        # Pillowロードチェック
        with Image.open(out_file) as img_check:
            img_check.verify()
        with Image.open(out_file) as img_check:
            img_check.load()
            w, h = img_check.size
        ...
        # DBへの結果保存とマイグレーションの検証
        conn = sqlite3.connect(str(db_file))
        try:
            cursor = conn.execute("SELECT status, result FROM tasks WHERE id = ?", (task_id,))
            status, result_str = cursor.fetchone()
            assert status == "COMPLETED"
            
            # thumbnail_results テーブルとマイグレーションカラム verified_at の検証
            cursor_res = conn.execute("SELECT task_id, path, width, height, size_bytes, verified_at FROM thumbnail_results WHERE task_id = ?", (task_id,))
            row_res = cursor_res.fetchone()
            assert row_res is not None
...
@pytest.mark.asyncio
async def test_thumbnail_generator_exponential_backoff_and_jitter():
    """
    検証基準: 
    - 429 などのリトライ可能な APIError が発生した際、指数バックオフと Jitter を用いて最大3回リトライされること
    """
    from google.genai.errors import APIError
    from unittest.mock import patch, MagicMock
    generator = ThumbnailGenerator()
    ...
    # 3回とも APIError(429) を投げる
    err_429 = APIError(429, "Rate Limit")
    
    with patch.object(generator.client.models, "generate_images", side_effect=err_429) as mock_gen, \
         patch("asyncio.sleep") as mock_sleep:
        
        result = await generator._generate_image("Test Prompt")
        assert result is None
        assert mock_gen.call_count == 3
        assert mock_sleep.call_count == 2
        
        # 指数バックオフの秒数チェック (2**attempt + jitter)
        # 1回目の sleep: 2**1 = 2秒以上
        # 2回目の sleep: 2**2 = 4秒以上
        called_args = [call.args[0] for call in mock_sleep.call_args_list]
        assert called_args[0] >= 2.0
        assert called_args[1] >= 4.0

@pytest.mark.asyncio
async def test_thumbnail_generator_non_retryable_fatal_error():
    """
    検証基準: 
    - 400 などの致命的な APIError が発生した際、リトライせずに即座に処理を中断すること
    """
    from google.genai.errors import APIError
    from unittest.mock import patch, MagicMock
    generator = ThumbnailGenerator()
    ...
    err_400 = APIError(400, "Invalid Argument")
    
    with patch.object(generator.client.models, "generate_images", side_effect=err_400) as mock_gen, \
         patch("asyncio.sleep") as mock_sleep:
        
        result = await generator._generate_image("Test Prompt")
        assert result is None
        assert mock_gen.call_count == 1 # 1回で打ち切り
        assert not mock_sleep.called # sleepされない

@pytest.mark.asyncio
async def test_thumbnail_generator_adaptive_contrast_compensation():
    """
    検証基準:
    - 画像の輝度標準偏差(std_dev)が低い（コントラストが低い）場合に、より強いコントラスト補正が適用されること
    - 高コントラストな画像では穏やかな補正が適用されること
    """
    generator = ThumbnailGenerator()
    
    # 1. 低コントラストな画像 (全体がほぼ同じ色のグレー)
    img_low_contrast = Image.new("RGB", (1280, 720), color=(128, 128, 128))
    import io
    img_io = io.BytesIO()
    img_low_contrast.save(img_io, format="JPEG")
    low_contrast_bytes = img_io.getvalue()
    
    result_low = generator.verify_and_optimize_image(low_contrast_bytes, title="Low Contrast Test")
    assert result_low is not None
    
    # 2. 高コントラストな画像 (白黒のストライプなど)
    img_high_contrast = Image.new("RGB", (1280, 720), color=(0, 0, 0))
    from PIL import ImageDraw
    draw = ImageDraw.Draw(img_high_contrast)
    draw.rectangle([0, 0, 640, 720], fill=(255, 255, 255))
    img_io_high = io.BytesIO()
    img_high_contrast.save(img_io_high, format="JPEG")
    high_contrast_bytes = img_io_high.getvalue()
    
    result_high = generator.verify_and_optimize_image(high_contrast_bytes, title="High Contrast Test")
    assert result_high is not None


@pytest.mark.asyncio
async def test_thumbnail_generator_truncated_image_handling():
    """
    検証基準: 
    - 切り詰められた（データ途切れ）画像データが入力された場合の動作検証。
    - 一部データが欠損していても、ImageFile.LOAD_TRUNCATED_IMAGES の効果でデコードできるか、
      あるいは破損が深刻な場合は安全にフォールバック画像が生成され、例外を投げないこと。
    """
    generator = ThumbnailGenerator()
    
    # 正常な画像を作成してJPEGバイナリを取得
    ...
    # 意図的にバイナリの後半部分を切り落として「切り詰め画像」を作成
    truncated_bytes = original_bytes[:len(original_bytes) // 2]
    
    # verify_and_optimize_image を実行。例外が発生しないこと。
    optimized_bytes = generator.verify_and_optimize_image(truncated_bytes, title="Truncated Image Test")
    
    assert optimized_bytes is not None
    assert len(optimized_bytes) > 0
    
    # デコードされた画像が正常な形式であることを確認
    img_check = Image.open(io.BytesIO(optimized_bytes))
    img_check.load()
    assert img_check.size[0] >= 1280
    assert img_check.size[1] >= 720


@pytest.mark.asyncio
async def test_thumbnail_generator_partial_variant_failure():
    """
    検証基準:
    - 複数 variants の生成時、一部の variant 生成で APIError が発生しても、
      全体を失敗させず、失敗した variant はフォールバック画像で代替され、
      最終的に要求された件数（3つ）の結果が返ってくること。
    """
    from google.genai import errors
    from unittest.mock import patch, MagicMock
    generator = ThumbnailGenerator()
    ...
    # generate_images が 1回目と3回目は正常レスポンス、2回目は APIError を吐くように設定
    mock_response = MagicMock()
    mock_image = MagicMock()
    img = Image.new("RGB", (1280, 720), color=(100, 100, 255))
    import io
    ...
    api_err = errors.APIError(500, "Temporary API Failure")
    
    # side_effect に正常、エラー(3回リトライ)、正常の順で登録
    side_effects = [mock_response, api_err, api_err, api_err, mock_response]
    
    with patch.object(generator.client.models, "generate_images", side_effect=side_effects) as mock_gen, \
         patch.object(generator, "_get_brand_style", return_value="Test brand style"), \
         patch.object(generator, "_generate_concepts", return_value=[
             {"name": "Concept 1", "description": "Desc 1", "visual_prompt": "Prompt 1", "expected_ctr": 8.0},
             {"name": "Concept 2", "description": "Desc 2", "visual_prompt": "Prompt 2", "expected_ctr": 7.0},
             {"name": "Concept 3", "description": "Desc 3", "visual_prompt": "Prompt 3", "expected_ctr": 6.0}
         ]), \
         patch("asyncio.sleep") as mock_sleep:
         
        results = await generator.generate(video_title="Partial Failure Test", num_variants=3)
        
        # 3つのコンセプトすべてに対して結果（フォールバック含む）が返っていること
        ...
        assert results[1]["concept_name"] == "Concept 2"
        assert results[2]["concept_name"] == "Concept 3"
...
@pytest.mark.asyncio
async def test_thumbnail_generator_gradient_theme_selection():
    """
    検証基準:
    - 異なるタイトルが指定された際に、タイトルハッシュに基づいて異なるグラデーション配色テーマが適用されること。
    - 生成されたフォールバック画像同士のバイナリが異なることを確認する。
    """
    generator = ThumbnailGenerator()
    
    # タイトルによってハッシュ値が異なるように設定
    title_a = "Cyberpunk Future Tech"
    title_b = "Warm Amber Cooking Class"
    
    # 破損画像を入力してフォールバック画像（グラデーション）を生成させる
    bytes_a = generator.verify_and_optimize_image(b"corrupted_data", title=title_a)
    bytes_b = generator.verify_and_optimize_image(b"corrupted_data", title=title_b)
    
    assert bytes_a is not None
    assert bytes_b is not None
    
    # 両画像が異なるバイナリであることを確認（配色が異なるため）
    assert bytes_a != bytes_b
    
    # それぞれ正常な画像としてロードできること
    import io
...
# ====================================================
# 追加のテストケース (画像品質とエラーハンドリングの強化検証用)
# ====================================================

@pytest.mark.asyncio
async def test_thumbnail_generator_neon_shadow_fallback_custom():
    """
    検証基準:
    - 新しく実装された「ぼかし付きドロップシャドウ (GaussianBlur)」「新規プレミアムカラーテーマ」が
      フォールバック画像生成時に正しく呼び出され、例外を発生させずに高品質な画像が生成されること。
    - 生成された画像が正しいフォーマット (JPEG) であり、Pillowでロードできること。
    """
    generator = ThumbnailGenerator()
    
    # 新規テーマをターゲットにするため、複数のタイトルでテスト
    titles = [
        "Gold Luxury Amethyst Rose",
        "Deep Royal Green Mint",
        "Crimson Diamond Salmon Pink"
    ]
    
    for title in titles:
        # 破損画像を入れてフォールバック画像生成を走らせる
        result_bytes = generator.verify_and_optimize_image(b"corrupt_image_data", title=title)
        assert result_bytes is not None
        assert result_bytes.startswith(b"\xff\xd8") # JPEG header
...
@pytest.mark.asyncio
async def test_thumbnail_generator_extreme_aspect_ratio_correction():
    """
    検証基準:
    - アスペクト比が極端な入力（1:50の超縦長、50:1の超横長）に対しても、
      リサイズや中央クロップ処理がクラッシュせずに 16:9 且つ 1280x720 以上の適切な解像度に補正されること。
    """
    generator = ThumbnailGenerator()
    import io
    
    # 1. 1:50 の極端な縦長画像 (20x1000)
    img_tall = Image.new("RGB", (20, 1000), color=(128, 0, 0))
    img_io_t = io.BytesIO()
    img_tall.save(img_io_t, format="JPEG")
    tall_bytes = img_io_t.getvalue()
    
    optimized_tall = generator.verify_and_optimize_image(tall_bytes, title="Extreme Tall Test")
    img_check_t = Image.open(io.BytesIO(optimized_tall))
    img_check_t.load()
    w_t, h_t = img_check_t.size
    assert w_t >= 1280
    assert h_t >= 720
    assert abs((w_t / h_t) - (16.0 / 9.0)) < 0.01
    
    # 2. 50:1 の極端な横長画像 (1000x20)
    img_wide = Image.new("RGB", (1000, 20), color=(0, 128, 0))
    img_io_w = io.BytesIO()
    img_wide.save(img_io_w, format="JPEG")
    wide_bytes = img_io_w.getvalue()
    
    optimized_wide = generator.verify_and_optimize_image(wide_bytes, title="Extreme Wide Test")
    img_check_w = Image.open(io.BytesIO(optimized_wide))
    img_check_w.load()
    w_w, h_w = img_check_w.size
    assert w_w >= 1280
    assert h_w >= 720
    assert abs((w_w / h_w) - (16.0 / 9.0)) < 0.01

@pytest.mark.asyncio
async def test_thumbnail_generator_explicit_quality_compliance_rules():
    """
    最優先ルールで定められた以下の基準を個別に明示的にアサーションして検証する:
    - 解像度が 1280x720 以上であること
    - アスペクト比が 16:9 であること
    - ファイルサイズが 4MB 未満であること
    - 出力ファイルが正常に存在し、破損していない（Pillow等で正常にロード可能である）こと
    ...
    # 正常な画像生成結果から最適化を行うシミュレーション
    img_src = Image.new("RGB", (1920, 1080), color=(30, 30, 80))
    import io
    img_io = io.BytesIO()
    img_src.save(img_io, format="JPEG")
    src_bytes = img_io.getvalue()
    
    # 最適化実行
    final_bytes = generator.verify_and_optimize_image(src_bytes, title="Compliance Verification")
    
    # 1. 出力ファイルが正常に存在し、空ではないこと
    assert final_bytes is not None
    assert len(final_bytes) > 0
    
    # 2. ファイルサイズが 4MB 未満であること (厳密には 2MB 未満に制御)
    assert len(final_bytes) < 4 * 1024 * 1024
    
    # 3. 破損していないこと (Pillowでロードおよびverify可能であること)
    img_dest = Image.open(io.BytesIO(final_bytes))
    img_dest.verify()
    
    img_dest = Image.open(io.BytesIO(final_bytes))
    img_dest.load()
    width, height = img_dest.size
    
    # 4. 解像度が 1280x720 以上であること
    assert width >= 1280
    assert height >= 720
    
    # 5. アスペクト比が 16:9 であること (許容誤差 1%)
    aspect_ratio = width / height
    target_ratio = 16.0 / 9.0
    assert abs(aspect_ratio - target_ratio) < 0.01

@pytest.mark.asyncio
async def test_resolve_task_custom_error_handling_io_and_value(tmp_path):
    """
    検証基準:
    - resolve_generator_thumbnail_task 内で IOError や ValueError が発生した際、
      それぞれに応じた個別の技術負債 (TechnicalDebtStore) 登録ルートを通り、例外が適切に再送出されること。
    """
    db_file = tmp_path / "test_custom_errors.db"
    agent = StageBoundAgent(stage_name="thumbnail", db_path=str(db_file))
    agent.video_title = "Error Test Title"
    agent.db_path = str(db_file)
    agent.output_dir = str(tmp_path)
    
    # 1. ValueError をスローさせる (image_base64 が空)
    with patch.object(ThumbnailGenerator, "generate", return_value=[{"image_base64": ""}]), \
         patch("agents.memory.technical_debt.TechnicalDebtStore.register_debt") as mock_tdr_val:
         
        with pytest.raises(ValueError, match="Generated thumbnail contains no image data"):
            await resolve_generator_thumbnail_task(agent, "task_val_error")
        # ValueError 用の TDR 登録が走ったか確認
        assert mock_tdr_val.called
        assert mock_tdr_val.call_args[1].get("category") == "IMAGE_QUALITY_PIPELINE"

    # 2. IOError をスローさせる (書き込み失敗)
    img = Image.new("RGB", (1280, 720), color=(100, 100, 100))
    import io
    import base64
    img_io = io.BytesIO()
    ...
    # rename と move を両方失敗させて IOError を発生させる
    with patch.object(ThumbnailGenerator, "generate", return_value=[{"image_base64": mock_base64}]), \
         patch("pathlib.Path.rename", side_effect=OSError("Rename failed")), \
         patch("shutil.move", side_effect=IOError("Move failed")), \
         patch("agents.memory.technical_debt.TechnicalDebtStore.register_debt") as mock_tdr_io:
         
        with pytest.raises(IOError):
            await resolve_generator_thumbnail_task(agent, "task_io_error")
        # IOError 用の TDR 登録が走ったか確認
        assert mock_tdr_io.called
        assert mock_tdr_io.call_args[1].get("category") == "DISK_IO_STABILITY"
