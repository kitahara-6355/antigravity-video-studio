...
class TestWhisperSubprocess:
    ...
    def test_main_success_flow(self, tmp_path, capsys):
        ...
        with patch("sys.argv", ["whisper_subprocess.py", str(video), str(output), "small", "ja"]):
            with patch.dict("sys.modules", {"ctranslate2": MagicMock(get_supported_compute_types=MagicMock(side_effect=RuntimeError("no CUDA")))}):
    ...
    # WH-13: get_video_hash一意性検証
    def test_get_video_hash(self):
        from subtitle_engine.whisper_subprocess import get_video_hash
        h1 = get_video_hash("video1.mp4")
        h2 = get_video_hash("video2.mp4")
        assert h1 != h2
        assert len(h1) == 12

    # WH-14: extract_audio_wavの一意ファイル名＆再利用
    def test_extract_audio_hash_named(self, tmp_path):
        from subtitle_engine.whisper_subprocess import extract_audio_wav, get_video_hash
        video = tmp_path / "video.mp4"
        video.write_bytes(b"fake")
        mock_result = MagicMock(returncode=0, stderr="")
        video_hash = get_video_hash(str(video))
        expected_wav = tmp_path / f"_whisper_audio_{video_hash}.wav"

        def _create_wav(*args, **kwargs):
            expected_wav.write_bytes(b"x" * 2048)
            return mock_result

        with patch("subprocess.run", side_effect=_create_wav):
            result = extract_audio_wav(str(video), str(tmp_path))
        assert result == str(expected_wav)
        assert expected_wav.exists()

    # WH-15: split_wav_chunks一意チャンク名
    def test_split_wav_multiple_chunks_hash_named(self, tmp_path):
        from subtitle_engine.whisper_subprocess import split_wav_chunks
        wav = tmp_path / "long.wav"
        wav.write_bytes(b"x" * 2000)

        call_count = [0]
        video_hash = "abc123test"
        ...
        def mock_run(*args, **kwargs):
            ...
            if call_count[0] == 1:
                ...
                cmd_args = args[0]
                for arg in cmd_args:
                    if str(arg).endswith(".wav") and f"_chunk_{video_hash}_" in str(arg):
                        Path(arg).write_bytes(b"x" * 2000)
                        break
                return MagicMock(returncode=0)

        with patch("subprocess.run", side_effect=mock_run):
            chunks = split_wav_chunks(str(wav), str(tmp_path), chunk_sec=300, video_hash=video_hash)
        assert len(chunks) == 2
        for chunk_path, _, _ in chunks:
            assert f"_chunk_{video_hash}_" in chunk_path

    # WH-16: None値や属性欠損に対するロバスト性
    def test_transcribe_chunk_robustness_with_nones(self, tmp_path):
        from subtitle_engine.whisper_subprocess import transcribe_chunk
        
        class FakeWord:
            def __init__(self, word=None, start=0.0, end=1.0):
                self.word = word
                self.start = start
                self.end = end

        class FakeSegment:
            def __init__(self, text=None, start=0.0, end=2.0, words=None):
                self.text = text
                self.start = start
                self.end = end
                self.words = words

        mock_seg = FakeSegment(text=None, start=0.0, end=2.0, words=[FakeWord(word=None, start=0.0, end=1.0)])
        ...
        assert result[0]["words"][0]["word"] == ""
        assert result[0]["words"][0]["start"] == 10.0

    # WH-17: get_video_hashでの例外発生時のフォールバック処理
    def test_get_video_hash_exception(self):
        from subtitle_engine.whisper_subprocess import get_video_hash
        with patch("os.path.abspath", side_effect=ValueError("mock abs path error")):
            h = get_video_hash("video.mp4")
            assert len(h) == 12

    # WH-18: mainでのWhisperModelロード時の例外発生処理とフォールバック、およびチャンクスキップ
    def test_main_model_load_exception(self, tmp_path):
        from subtitle_engine.whisper_subprocess import main
        video = tmp_path / "video.mp4"
        video.write_bytes(b"fake")
        output = tmp_path / "output.jsonl"
        ...
        call_count = [0]
        def mock_whisper_init(model_size, device, compute_type):
            call_count[0] += 1
            if call_count[0] == 1:
                raise RuntimeError("GPU load error")
            return MagicMock()
        ...
        with patch("sys.argv", ["whisper_subprocess.py", str(video), str(output), "small", "ja"]):
            with patch("subprocess.run", return_value=mock_probe):
                with patch("subtitle_engine.whisper_subprocess.extract_audio_wav", return_value="fake.wav"):
                    with patch("subtitle_engine.whisper_subprocess.split_wav_chunks", return_value=[("fake.wav", 0.0, 30.0)]):
                        with patch("subtitle_engine.whisper_subprocess.transcribe_chunk", return_value=[]):
                            with patch("os._exit", mock_exit):
                                import subtitle_engine.whisper_subprocess as ws_mod
                                with patch.dict("sys.modules", {"ctranslate2": MagicMock()}):
                                    with patch.dict("sys.modules", {"faster_whisper": MagicMock(WhisperModel=mock_whisper_init)}):
                                        ws_mod.main()
        mock_exit.assert_called_with(0)

    # WH-19: chunk削除時の例外発生時の処理
    def test_main_chunk_delete_exception(self, tmp_path):
        from subtitle_engine.whisper_subprocess import main
        video = tmp_path / "video.mp4"
        video.write_bytes(b"fake")
        output = tmp_path / "output.jsonl"
        ...
        with patch("sys.argv", ["whisper_subprocess.py", str(video), str(output), "small", "ja"]):
            with patch("subprocess.run", return_value=mock_probe):
                with patch("subtitle_engine.whisper_subprocess.extract_audio_wav", return_value="fake.wav"):
                    with patch("subtitle_engine.whisper_subprocess.split_wav_chunks", return_value=[("chunk_1.wav", 0.0, 30.0)]):
                        with patch("subtitle_engine.whisper_subprocess.transcribe_chunk", return_value=[{"start": 0.0, "end": 2.0, "text": "hello"}]):
    ...
    # WH-20: mainでのCUDA検出成功処理
    def test_main_cuda_success(self, tmp_path):
        video = tmp_path / "video.mp4"
        video.write_bytes(b"fake")
        output = tmp_path / "output.jsonl"
        mock_exit = MagicMock()
    ...
    # WH-21: mainでのGPUロードタイムアウトCPUフォールバック処理
    def test_main_model_load_timeout(self, tmp_path):
        video = tmp_path / "video.mp4"
        video.write_bytes(b"fake")
        output = tmp_path / "output.jsonl"
        mock_exit = MagicMock()
                        ...
                        with patch("subtitle_engine.whisper_subprocess.split_wav_chunks", return_value=[(str(wav_path), 0.0, 30.0)]):
                            with patch("subtitle_engine.whisper_subprocess.transcribe_chunk", return_value=[{"start": 0.0, "end": 2.0, "text": "hello"}]):
                                with patch("os._exit", mock_exit):
                                    ...
                                    mock_whisper_cls = MagicMock(return_value=mock_cpu_model)
                                    with patch.dict("sys.modules", {"faster_whisper": MagicMock(WhisperModel=mock_whisper_cls)}):
                                        ws_mod.main()
                                    
        mock_exit.assert_called_with(0)

    # WH-22: NVIDIA DLLパスの追加処理
    def test_nvidia_dll_path_injection(self, tmp_path):
        import importlib
        import sys
        ...
        for sub in ["cublas/bin", "cudnn/bin", "cuda_nvrtc/bin"]:
            (nvidia_dir / sub).mkdir(parents=True, exist_ok=True)
        
        fake_site_package = str(tmp_path / "site-packages")
        os.mkdir(fake_site_package)
        real_nvidia_dir = Path(fake_site_package) / "nvidia"
        real_nvidia_dir.mkdir()
        for sub in ["cublas/bin", "cudnn/bin", "cuda_nvrtc/bin"]:
            (real_nvidia_dir / sub).mkdir(parents=True, exist_ok=True)
            
        sys.path.append(fake_site_package)
        mock_exit = MagicMock()
        try:
            with patch("sys.argv", ["whisper_subprocess.py", "dummy_video.mp4", "dummy_output.jsonl"]):
                with patch("os._exit", mock_exit):
                    with patch("subprocess.run", side_effect=RuntimeError("abort after DLL injection")):
                        with patch.dict(os.environ, {"PATH": ""}):
                            import subtitle_engine.whisper_subprocess as ws_mod
                            ws_mod.main()
                            
                            path_env = os.environ.get("PATH", "")
                            assert "cublas" in path_env
                            assert "cudnn" in path_env
                            assert "cuda_nvrtc" in path_env
        finally:
            if fake_site_package in sys.path:
                sys.path.remove(fake_site_package)

    # WH-23: if __name__ == "__main__": main() の実行カバー
    def test_main_execution_as_script(self):
        import runpy
        with patch("sys.argv", ["whisper_subprocess.py"]):
            try:
                runpy.run_module("subtitle_engine.whisper_subprocess", run_name="__main__")
...
class TestDesignTokenManager:
    ...
    def dtm_env(self, tmp_path):
        """tmp_pathベース of DesignTokenManagerを構築"""
