# Flash専用セッション 起動指示プロンプト
**生成日時**: 2026-06-06 07:20 JST
...
- `タイマー`: 「300秒設定済み」「未設定⚠️」「120秒先行設定済み」のいずれかを明示
...
- **モード**: WEEKEND（休日日中）
- **MAX_PARALLEL (batch_size)**: 10件
- **タイマー間隔**: 300秒
- **アーカイブ判定**: コンテキスト予測 70%超過で自動warn（ハードキャップ: 35バッチ / 6時間）
- **ステータス出力**: full
- **サブエージェントTMO**: 600秒
- **バッチTMO**: 900秒
- **コンテキスト推定**: 4%/バッチ

**MAX_PARALLEL の適用**: `get_next_batch()` 呼出時に `batch_size=10` を使用すること。
**タイマーの適用**: バッチ間のウェイクアップタイマーは `300秒` を使用すること。
...
### Opus Directive（戦略指示）
- **Directive ID**: `D-opus-session-p30-gate-focus-v1`
- **優先度配分（自動進化同期）**: `test_weaver`: 35%, `bug_hunter`: 25%, `refactor`: 15%, `tdr_cleanup`: 15%, `thumbnail`: 5%, `design_stock`: 5%
- **重点モジュール**: `orchestrator.py`, `preview_report_generator.py`, `themes_router.py`, `youtuber_grade_scorer.py`, `quality_feedback_trigger.py`, `video_pipeline.py`, `projects_router.py`
- **戦略メモ**: Phase 30 Gate Focus: coverage 26.2→30%(+3.8%), test_count 1641→1700(+59). test_weaver 15→35%に大幅増（Gate通過の最短経路）。bug_hunter 25%維持（82%打率で高効率）。thumbnail/design_stock を一時縮小。focus_modulesにカバレッジ低モジュール（video_pipeline, projects_router）追加。

### バッチ状態
- **現在バッチID**: `batch_3903f2`
- **タスク状態**: pending=0, running=0, pass=12, fail=0
- **サムネイルタスク状態**: 合計=1件 (pending=0, running=0, pass=1, fail=0)

### セッション履歴
前回セッション実績: 8タスク完了 / 1バッチ処理

### ユーザー着席スケジュール
- **本日**: 土曜日 — 休日（終日対応モード）
- **着席窓**: 09:00-23:00 (終日対応)
- **休日ルール**: 09:00-23:00の間、1時間に1回チェック可能。「次回着席推奨」は不要、代わりに「終日対応窓内 — 随時確認可能」と記載すること
...
(Opus conversation ID未設定のためスキップ)
