...
@pytest.mark.asyncio
async def test_pipeline_tools_invalid_args_types():
    """引数 args が dict ではない無効な型だった場合、クラッシュせずにエラーになることを検証"""
    res = await tool_registry.execute("transcribe_video", "not_a_dict_but_a_string")
    assert res.is_error is True
    assert "video_path" in res.content[0]["text"] or "Tool execution failed" in res.content[0]["text"]


@pytest.mark.asyncio
async def test_pipeline_tools_transcribe_exception_handling(mock_sessions, mock_workers):
    """worker.execute 中に例外が発生した場合に、適切に例外をキャッチしてエラーを返すことを検証"""
    video_path = "dummy_video_path.mp4"
    # execute 内で例外を発生させる
    mock_workers["transcribe"].return_value.execute.side_effect = AttributeError("Mock AttributeError")
    
    res = await tool_registry.execute("transcribe_video", {"video_path": video_path})
    assert res.is_error is True
    assert "AttributeError" in res.content[0]["text"]
