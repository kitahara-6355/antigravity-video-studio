...
def create_valid_test_image(width=1280, height=720, fmt="PNG") -> Image.Image:
    return Image.new("RGB", (width, height), color=(60, 100, 60))

def test_generate_pipeline_coordinator_thumbnail_success(tmp_path):
    out_path = tmp_path / "test_thumb.png"
    res_path = generate_pipeline_coordinator_thumbnail(out_path)
...
def test_validate_pipeline_coordinator_thumbnail_success(tmp_path):
    ...
    result = validate_pipeline_coordinator_thumbnail(out_path)
    assert result["width"] == 1280
    assert result["height"] == 720
    assert result["path"] == str(out_path)
    assert result["size_bytes"] > 0

def test_validate_pipeline_coordinator_thumbnail_failures(tmp_path):
    # 1. 存在しないファイル
    with pytest.raises(FileNotFoundError):
        validate_pipeline_coordinator_thumbnail(tmp_path / "missing.png")
    ...
    with pytest.raises(ValueError, match="Resolution must be at least 1280x720"):
        validate_pipeline_coordinator_thumbnail(low_res_path)
    ...
    with pytest.raises(ValueError, match="Aspect ratio must be 16:9"):
        validate_pipeline_coordinator_thumbnail(bad_ratio_path)
    ...
    with patch("pathlib.Path.stat") as mock_stat:
        ...
        with pytest.raises(ValueError, match="File size exceeds 4MB limit"):
            validate_pipeline_coordinator_thumbnail(valid_path)

    # 5. 破損画像 (verify / load 失敗)
    corrupt_path = tmp_path / "corrupt.png"
    # 空ファイルを作成して画像破損をシミュレート
    with open(corrupt_path, "wb") as f:
        f.write(b"corrupted data")
    with pytest.raises(ValueError, match="Image is corrupted or invalid format"):
        validate_pipeline_coordinator_thumbnail(corrupt_path)

def test_resolve_pipeline_coordinator_thumbnail_task_stage_bound(tmp_path):
    db_file = tmp_path / "test_pipeline_coordinator_thumb.db"
    task_id = "pipeline_coordinator_thumb_test"
    ...
    async def run_test():
        ...
        # agents.pipeline_coordinator.THUMBNAIL_OUTPUT_DIR を tmp_path に一時的にパッチ
        import agents.pipeline_coordinator
        with patch.object(agents.pipeline_coordinator, "THUMBNAIL_OUTPUT_DIR", tmp_path):
            output_file = tmp_path / f"{task_id}.png"
            
            await agent.start(resolve_pipeline_coordinator_thumbnail_task)
