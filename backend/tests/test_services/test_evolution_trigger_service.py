...
def test_specific_exceptions_caught():
    """_load_constitution が json.JSONDecodeError や OSError はキャッチし、予期せぬ ValueError などは上にスルーすることを検証"""
    service = EvolutionTriggerService()
    
    # json.load が ValueError を投げるようにモックする
    with mock.patch("json.load", side_effect=ValueError("Unexpected Error")):
        with mock.patch("builtins.open", mock.mock_open(read_data="{}")):
            with mock.patch("pathlib.Path.exists", return_value=True):
                with pytest.raises(ValueError) as excinfo:
                    service._load_constitution()
