...
def test_score_helper_methods():
    engine = SoulFeedbackEngine(analytics_dir=None, constitution_path=None)
    # _score_ctr の境界値
    assert engine._score_ctr(8.5) == 20.0
    assert engine._score_ctr(7.0) == 10.0
    assert engine._score_ctr(5.0) == 5.0
    assert engine._score_ctr(3.0) == 0.0

    # _score_retention の境界値
    assert engine._score_retention(75.0) == 20.0
    assert engine._score_retention(60.0) == 10.0
    assert engine._score_retention(45.0) == 5.0
    assert engine._score_retention(30.0) == 0.0

    # _score_deviation の境界値
    assert engine._score_deviation(15.0) == 10.0
    assert engine._score_deviation(30.0) == 5.0
    assert engine._score_deviation(50.0) == 0.0

    # _score_patterns の境界値
    assert engine._score_patterns(12) == 5.0
    assert engine._score_patterns(6) == 2.5
    assert engine._score_patterns(3) == 0.0
...
def test_parse_llm_suggestions():
    ...
    ```"""
    suggestions = engine._parse_llm_suggestions(json_text)
    assert len(suggestions) == 1
    assert suggestions[0].category == "テンポ"
    assert suggestions[0].suggestion == "15秒にフック"
    ...
    suggestions = engine._parse_llm_suggestions(json_text_raw)
    assert len(suggestions) == 1
    assert suggestions[0].category == "ビジュアル"


def test_get_gemini_client_safely(monkeypatch):
    engine = SoulFeedbackEngine(analytics_dir=None, constitution_path=None)
    ...
    dummy_factory = types.ModuleType("gemini_client_factory")
    dummy_factory.get_gemini_client = lambda: DummyClient()
    sys.modules["gemini_client_factory"] = dummy_factory
    
    client = engine._get_gemini_client_safely()
    assert isinstance(client, DummyClient)
    
    # 例外系
    def raise_err():
        raise Exception("test_error")
    dummy_factory.get_gemini_client = raise_err
    client_err = engine._get_gemini_client_safely()
