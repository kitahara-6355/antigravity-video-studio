...
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))); import check_pipeline_status as check_pipeline_status
...
async def test_resolve_thumbnail_task_directory_traversal():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        with patch("check_pipeline_status.OUTPUT_DIR", tmpdir):
...
async def test_resolve_thumbnail_task_special_char_task_id():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        with patch("backend.check_pipeline_status.OUTPUT_DIR", tmpdir):
            invalid_task_id = "task_<>:\"|?*"
            with pytest.raises(OSError):
                await check_pipeline_status.resolve_pipeline_status_thumbnail_task(invalid_task_id)
...
def test_print_result_non_boolean_success_value(capsys):
    # エッジケース: successが真偽値以外（例: truthyな文字列やfalsyな空リスト）の場合の挙動
    check_pipeline_status._print_result({
        "duration_seconds": 15.0,
        "final_path": "path/to/video.mp4",
        "quality_score": 80,
        "stage_results": [
            {"name": "stage_truthy", "success": "yes", "duration": 5.0},
            {"name": "stage_falsy", "success": [], "duration": 3.0}
        ]
    })
    captured = capsys.readouterr()
    assert "✅ stage_truthy: 5.0s" in captured.out
    assert "❌ stage_falsy: 3.0s" in captured.out

def test_print_stages_iterable_generator(capsys):
    # エッジケース: stagesにリストではなくジェネレータオブジェクトを渡した場合の挙動
    def stage_gen():
        yield {"index": 0, "name": "first_stage", "status": "completed"}
        yield {"index": 1, "name": "second_stage", "status": "running"}
    
    check_pipeline_status._print_stages(stage_gen())
    captured = capsys.readouterr()
    assert "✅ [0] first_stage: completed" in captured.out
    assert "🔄 [1] second_stage: running" in captured.out

def test_print_feedback_contains_dicts(capsys):
    # エッジケース: feedbackのリスト内に文字列以外のオブジェクト（辞書など）が含まれる場合の挙動
    check_pipeline_status._print_feedback([
        "Feedback string",
        {"detail": "detailed feedback"},
        [1, 2, 3]
    ])
    captured = capsys.readouterr()
    assert "Quality Feedback:" in captured.out
    assert "  - Feedback string" in captured.out
    assert "  - {'detail': 'detailed feedback'}" in captured.out
    assert "  - [1, 2, 3]" in captured.out

def test_fetch_pipeline_status_data_read_returns_str(capsys):
    # エッジケース: response.read() が bytes ではなく str を返して decode() で AttributeError が発生した場合
    mock_response = MagicMock()
    mock_response.read.return_value = "string content (not bytes)"
    ...
    assert excinfo.value.code == 1
    captured = capsys.readouterr()
    # TDR-CRITICAL: Unexpected JSON parse error safety net のパスを通る
    assert "Unexpected error during parsing" in captured.err

def test_validate_thumbnail_invalid_object_type():
    # エッジケース: validate_pipeline_status_thumbnail に Path または文字列に変換できない無効な型を渡した場合
    with pytest.raises(TypeError):
        check_pipeline_status.validate_pipeline_status_thumbnail([Path("some_path")])





# --- さらに追加のエッジケーステスト（境界値、None入力、空リスト、巨大入力、不正型） ---

def test_print_result_stage_results_invalid_type():
    # エッジケース: stage_results がリストではなく辞書や文字列などの不正な型の場合
    with pytest.raises(AttributeError):
        check_pipeline_status._print_result({
            "duration_seconds": 10.0,
            "final_path": "path",
            "quality_score": 100,
            "stage_results": "not_a_list"
        })
    with pytest.raises(AttributeError):
        check_pipeline_status._print_result({
            "duration_seconds": 10.0,
...
def test_print_result_quality_feedback_invalid_type():
    # エッジケース: quality_feedback がリストではなく整数（イテレート不可）の場合
    with pytest.raises(TypeError):
        check_pipeline_status._print_result({
            "duration_seconds": 10.0,
            "final_path": "path",
            "quality_score": 100,
            "quality_feedback": 123
        })

def test_generate_thumbnail_float_dimensions():
    # エッジケース: width / height に float 型を渡した場合 (int 変換されて成功するはず)
    with tempfile.TemporaryDirectory() as tmpdir:
        out_path = Path(tmpdir) / "thumb_float_val.png"
        res_path = check_pipeline_status.generate_pipeline_status_thumbnail(
            out_path, width=1280.0, height=720.0
        )
        assert res_path == out_path
        assert out_path.exists()
        with Image.open(out_path) as img:
...
def test_generate_thumbnail_invalid_text_type():
    # エッジケース: text に文字列ではなく辞書などの不正な型を渡した場合
    with tempfile.TemporaryDirectory() as tmpdir:
        out_path = Path(tmpdir) / "thumb_invalid_text.png"
        with pytest.raises(AttributeError):
            check_pipeline_status.generate_pipeline_status_thumbnail(
                out_path, text={"invalid": "text"}
            )

def test_validate_thumbnail_jpeg_format():
    # エッジケース: PNG以外の有効な画像形式 (JPEG) の場合でも検証が成功することを確認
    with tempfile.TemporaryDirectory() as tmpdir:
        out_path = Path(tmpdir) / "valid.jpg"
...
@pytest.mark.asyncio
async def test_resolve_thumbnail_task_non_string_task_id():
    # エッジケース: task_id に None や整数などの非文字列型を渡した場合
    with tempfile.TemporaryDirectory() as tmpdir:
        with patch("backend.check_pipeline_status.OUTPUT_DIR", tmpdir):
            result_json = await check_pipeline_status.resolve_pipeline_status_thumbnail_task(None)
            result = json.loads(result_json)
            assert Path(result["path"]).name == "None.png"
            assert Path(result["path"]).exists()
            
            result_json = await check_pipeline_status.resolve_pipeline_status_thumbnail_task(999)
            result = json.loads(result_json)
            assert Path(result["path"]).name == "999.png"
            assert Path(result["path"]).exists()

@pytest.mark.asyncio
async def test_resolve_thumbnail_task_dot_task_id():
    # エッジケース: task_id が "." の場合
    with tempfile.TemporaryDirectory() as tmpdir:
        with patch("backend.check_pipeline_status.OUTPUT_DIR", tmpdir):
            result_json = await check_pipeline_status.resolve_pipeline_status_thumbnail_task(".")
            result = json.loads(result_json)
            assert Path(result["path"]).name == "..png" or Path(result["path"]).name == ".png"
            assert Path(result["path"]).exists()


def test_print_stages_extreme_unusual_types(capsys):
    # エッジケース: stage_nameやdetailにカスタム文字列表現オブジェクトを渡した場合
    class CustomStr:
        def __str__(self):
            return "CustomStrValue"
    ...
    captured = capsys.readouterr()
    assert "✅ [CustomStrValue] CustomStrValue: completed CustomStrValue" in captured.out


def test_print_result_custom_truthy_success(capsys):
    # エッジケース: successがCustom __bool__ を持つオブジェクトの場合
    class CustomTruthy:
        def __bool__(self):
            return True
    class CustomFalsy:
    ...
    check_pipeline_status._print_result({
        ...
        "quality_score": 100,
        "stage_results": [
            {"name": "truthy_stage", "success": CustomTruthy(), "duration": 1.0},
            {"name": "falsy_stage", "success": CustomFalsy(), "duration": 2.0}
        ]
    })
    captured = capsys.readouterr()
    assert "✅ truthy_stage: 1.0s" in captured.out
    assert "❌ falsy_stage: 2.0s" in captured.out


def test_generate_thumbnail_huge_text_newlines():
    # エッジケース: textに多数の改行が含まれる場合のサムネイル生成
    with tempfile.TemporaryDirectory() as tmpdir:
        out_path = Path(tmpdir) / "thumb_newlines.png"
        text_with_newlines = "\n".join(["Line"] * 100)
        res_path = check_pipeline_status.generate_pipeline_status_thumbnail(
            out_path, text=text_with_newlines
        )
        assert res_path == out_path
        assert out_path.exists()
...
@pytest.mark.asyncio
async def test_resolve_thumbnail_task_control_chars_task_id():
    # エッジケース: task_idに制御文字が含まれる場合
    with tempfile.TemporaryDirectory() as tmpdir:
        with patch("backend.check_pipeline_status.OUTPUT_DIR", tmpdir):
            task_id = "task\x00_control"
            # ヌルバイトが含まれる場合はファイルパス操作で ValueError または OSError になるはず
            with pytest.raises((ValueError, OSError)):
                await check_pipeline_status.resolve_pipeline_status_thumbnail_task(task_id)


# --- さらなる追加のエッジケーステスト（境界値、None入力、空リスト、巨大入力、不正型） ---

def test_print_result_none_input(capsys):
    # エッジケース: resultにNoneを渡した場合、何も出力されず安全にreturnすること
    check_pipeline_status._print_result(None)
    captured = capsys.readouterr()
    assert captured.out == ""

def test_print_result_stage_results_duration_none():
    # エッジケース: stage_results内のdurationがNoneの場合、フォーマットエラー(TypeError)が発生すること
    with pytest.raises(TypeError):
        check_pipeline_status._print_result({
            "duration_seconds": 10.0,
            "final_path": "path",
            "quality_score": 100,
            "stage_results": [
                {"name": "encode", "success": True, "duration": None}
            ]
        })

def test_print_stages_element_none():
    # エッジケース: stagesのリスト内にNone要素が含まれる場合、AttributeErrorが発生すること
    with pytest.raises(AttributeError):
        check_pipeline_status._print_stages([None])

def test_print_result_quality_score_complex(capsys):
    # エッジケース: quality_scoreに辞書などの複雑なオブジェクトを渡した場合
    check_pipeline_status._print_result({
        "duration_seconds": 10.0,
        "final_path": "path",
        "quality_score": {"score": 95, "rank": "A"},
        "stage_results": []
    })
    captured = capsys.readouterr()
    assert "Quality:  {'score': 95, 'rank': 'A'}" in captured.out

def test_generate_thumbnail_non_writable_path():
    # エッジケース: 書き込み権限のない無効なパスやデバイス名を指定した場合にOSErrorが発生し、一時ファイルが残らないこと
    with tempfile.TemporaryDirectory() as tmpdir:
        out_path = Path(tmpdir) / "invalid_dir" / "thumb.png"
        # mkdirでOSErrorが発生するようにモック
        with patch("pathlib.Path.mkdir", side_effect=OSError("Mocked OS write failure")):
            with pytest.raises(OSError, match="Mocked OS write failure"):
                check_pipeline_status.generate_pipeline_status_thumbnail(out_path)
            # 一時ファイル (*.tmp) が残っていないことを確認
            tmp_files = list(Path(tmpdir).glob("*.tmp"))
            assert len(tmp_files) == 0
