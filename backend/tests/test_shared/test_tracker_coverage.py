...
def test_usage_record_init():
    record = UsageRecord(model="gemini-2.5-flash", requests=5, tokens_in=100, tokens_out=200)
...
def test_tracker_init_and_config(tmp_path):
    mock_config = {
        "free_tier_limits": {
            "gemini-2.5-flash": {"rpd": 100, "tier": "free"}
        },
        "alert_thresholds": {
            "warning": 0.75
        }
...
def test_load_or_create_daily_usage_exists(tmp_path):
    ...
    usage_data = {
        "date": today,
        "models": {
            "gemini-2.5-flash": {"requests": 5, "tokens_in": 50, "tokens_out": 50}
        }
    }
    ...
    assert tracker._daily_usage.get_requests("gemini-2.5-flash") == 5
...
def test_track_request_normal(tmp_path):
    ...
    mock_config = {
        "free_tier_limits": {
            "gemini-2.5-flash": {"rpd": 10, "tier": "free"}
        }
    }
    ...
    with patch("usage_tracker.tracker._load_model_config", return_value=mock_config):
        ...
        res = tracker.track_request("gemini-2.5-flash", tokens_in=10, tokens_out=20)
        ...
        res = tracker.track_request("gemini-2.5-flash", tokens_in=5, tokens_out=5)
        assert res["requests_today"] == 1
...
def test_can_make_request():
    tracker = UsageTracker()
    tracker._daily_usage = DailyUsage(date=date.today().isoformat())
    assert tracker.can_make_request("gemini-2.5-flash") is True
    
    tracker._daily_usage.models["gemini-2.5-flash"] = {"requests": 1000, "tokens_in": 0, "tokens_out": 0}
    assert tracker.can_make_request("gemini-2.5-flash") is False

def test_get_remaining_requests():
    tracker = UsageTracker()
    tracker._daily_usage = DailyUsage(date=date.today().isoformat())
    tracker._daily_usage.models["gemini-2.5-flash"] = {"requests": 350, "tokens_in": 0, "tokens_out": 0}
    assert tracker.get_remaining_requests("gemini-2.5-flash") == 650
...
def test_get_daily_summary():
    ...
    with patch("usage_tracker.tracker._load_model_config", return_value=mock_config):
        tracker = UsageTracker()
        tracker._daily_usage = DailyUsage(date=date.today().isoformat())
        tracker._daily_usage.models["gemini-2.5-flash"] = {"requests": 40, "tokens_in": 10, "tokens_out": 20}
        summary = tracker.get_daily_summary()
        
        assert "gemini-2.5-flash" in summary["models"]
        assert "gemini-2.5-pro" not in summary["models"]
        assert summary["models"]["gemini-2.5-flash"]["used"] == 40
        assert summary["models"]["gemini-2.5-flash"]["limit"] == 100
