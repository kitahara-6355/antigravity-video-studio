...
def test_run_gate_specific_exceptions(caplog):
    import re
    agent = QualityGateAgent()

    # AttributeErrorを発生させる
    def check_attribute_error(content):
        raise AttributeError("attribute_error")

    # IndexErrorを発生させる
    def check_index_error(content):
        raise IndexError("index_error")

    # re.errorを発生させる
    def check_re_error(content):
        raise re.error("re_error")
    ...
    with caplog.at_level(logging.ERROR):
        ...
        assert any("Unexpected quality check failure: check_index_error - index_error" in msg for msg in log_messages)
        assert any("Unexpected quality check failure: check_re_error - re_error" in msg for msg in log_messages)
