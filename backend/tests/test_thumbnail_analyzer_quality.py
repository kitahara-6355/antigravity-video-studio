...
def test_generate_thumbnail_4k_no_dither(tmp_path):
    """検証: 4Kなどの高解像度でディザリングなしで生成されることを確認"""
    out_path = tmp_path / "high_res_4k.png"
    thumbnail_analyzer.generate_thumbnail(out_path, width=3840, height=2160, text="4K Test")
    res = thumbnail_analyzer.validate_thumbnail(out_path)
    assert res["width"] == 3840
    assert res["size_bytes"] < 4 * 1024 * 1024

def test_validate_thumbnail_corrupted_magic_numbers(tmp_path):
    """検証: ファイルヘッダーと拡張子の不整合を検証"""
    out_path = tmp_path / "bad_magic.png"
    out_path.write_bytes(b"\x00" * 100) # PNGヘッダーではない
    with pytest.raises(ValueError, match="Image is corrupted or invalid format"):
        thumbnail_analyzer.validate_thumbnail(out_path)

    out_path_jpg = tmp_path / "bad_magic.jpg"
    out_path_jpg.write_bytes(b"\x00" * 100) # JPEGヘッダーではない
    with pytest.raises(ValueError, match="Image is corrupted or invalid format"):
        thumbnail_analyzer.validate_thumbnail(out_path_jpg)
