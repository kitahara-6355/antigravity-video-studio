...
def test_前提が宣言されている(canon):
    ...
    **ロードマップに残っているフェーズを全部見る。** 以前は ("R1.5", "R5") と名前で
    決め打ちしていて、R1.5 を phase_history へ移したとき（2026-09-19）に KeyError で落ちた。
    完了したフェーズは order から外れるので、残りはすべて未着手か準備中。
    """
    for pid in canon["roadmap"]["order"]:
