...
def _frontend(tmp_path: Path, source: str, extra: dict | None = None) -> Path:
    ...
    for name, text in (extra or {}).items():
        target = src / name
        # 呼び出し口は `gateway/` の下に置く。サブディレクトリを作れないと
        # 閉包側のテストが1つも書けない。
...
def _with_catalogue(tmp_path: Path, entries: str, *, source: str = "",
    ...
    files = dict(extra or {})
    files["gateway/endpoints.js"] = _CATALOGUE_HEAD + entries + "};\n"
    if client:
        files["gateway/client.js"] = client
    return _run(tmp_path, source or "export default function App() { return null; }",
                extra=files)


def _catalogue_sites(report):
    return [s for s in report.sites if s.file.endswith("gateway/endpoints.js")]
...
def test_a_dispatch_inside_the_gateway_is_not_an_unresolved_url(tmp_path):
    ...
    dispatch = [s for s in report.sites if s.file.endswith("gateway/client.js")]
...
def test_a_bare_origin_in_the_gateway_is_not_residual(tmp_path):
    ...
    assert [s for s in report.sites if s.file.endswith("gateway/client.js")] == []
...
def test_an_origin_with_a_path_in_the_gateway_is_residual(tmp_path):
    ...
    inside = [s for s in report.sites if s.file.endswith("gateway/client.js")]
...
def test_a_relative_import_specifier_is_not_a_backend_url(tmp_path):
    ...
    呼び出し口は `src/gateway/` に置いたのでこの衝突はもう起きないが、
    **他の相対 import に `/api/` が現れる余地は残る**。実測で見つけた
    誤検出（移行したファイル1つにつき unattributed が1件ずつ増える）を
    ここで固定しておく。
    """
    report = _with_catalogue(
        tmp_path, "  getStatus: { method: 'GET', path: '/api/demo/status' },\n",
...
def test_a_url_attribute_that_calls_the_gateway_is_resolved(tmp_path):
    ...
    report = _with_catalogue(
        tmp_path, "  getStatus: { method: 'GET', path: '/api/demo/status' },\n",
        source="""
            import { apiUrl } from '../gateway/client.js';
            export default function App() {
