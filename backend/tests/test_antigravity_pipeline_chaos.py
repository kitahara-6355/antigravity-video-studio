...
def test_parse_srt_timestamp_warning(tmp_path):
    ...
    with patch("antigravity_pipeline.logger.warning") as mock_warning:
        segments = pipeline._parse_srt(corrupt_srt)
...
def test_export_srt_temp_exists_false(temp_srt_file, tmp_path):
    """SRT出力用テンポラリファイルが存在しない場合のブランチテスト"""
    pipeline = AntigravityPipeline(output_dir=tmp_path / "output")
    original_exists = Path.exists
    
    def mock_exists(path_obj):
        if str(path_obj).endswith(".tmp_srt"):
            return False
    ...
    with patch.object(Path, "exists", autospec=True, side_effect=mock_exists):
        result = pipeline.process_srt(temp_srt_file)
        assert result["phases"]["srt_export"]["status"] == "completed"
        assert not Path(result["outputs"]["srt"]).exists()


def test_export_proposals_temp_exists_false(temp_srt_file, tmp_path):
    """提案レポート用テンポラリファイルが存在しない場合のブランチテスト"""
    pipeline = AntigravityPipeline(output_dir=tmp_path / "output")
    original_exists = Path.exists
    
    def mock_exists(path_obj):
        if str(path_obj).endswith(".tmp_json"):
            return False
        return original_exists(path_obj)
    ...
    with patch.object(Path, "exists", autospec=True, side_effect=mock_exists):
        result = pipeline.process_srt(temp_srt_file)
        assert result["phases"]["proposals_export"]["status"] == "completed"
        assert not Path(result["outputs"]["proposals"]).exists()


def test_quality_scoring_trigger_result_none_or_false(temp_srt_file, tmp_path):
    """NHKスコアラトリガーの結果がNone/Falseの場合のブランチテスト"""
    pipeline = AntigravityPipeline(output_dir=tmp_path / "output")
    with patch("services.nhk_quality_scorer.NHKQualityScorer") as mock_scorer,          patch("agents.orchestration.OrchestrationHub") as mock_hub_class:
        ...
        # trigger_quality_fix で None を返すようにする
        mock_hub_instance = mock_hub_class.return_value
        mock_hub_instance.trigger_quality_fix.return_value = None
...
def test_normalize_subtitles_prev_seg_invalid_structure(tmp_path):
    """_normalize_subtitles_for_quality 内で前セグメントが不正な構造の場合のブランチテスト"""
    pipeline = AntigravityPipeline(output_dir=tmp_path / "output")
    
    # 1. prev_seg が dict ではないケース
    segments_prev_not_dict = [
        "not a dict",
        {"id": "seg_002", "start": 1.0, "end": 1.1, "text": "非常に長いテキストで補正を発生させます"}
    ]
    
    # 2. prev_seg に end キーがないケース
    segments_prev_no_end = [
        {"id": "seg_001", "start": 0.0, "text": "OK"},
        {"id": "seg_002", "start": 1.0, "end": 1.1, "text": "非常に長いテキストで補正を発生させます"}
    ]
    
    # 3. prev_seg の end が None のケース
    segments_prev_end_none = [
        {"id": "seg_001", "start": 0.0, "end": None, "text": "OK"},
        {"id": "seg_002", "start": 1.0, "end": 1.1, "text": "非常に長いテキストで補正を発生させます"}
    ]
    
    normalized1 = pipeline._normalize_subtitles_for_quality(segments_prev_not_dict)
    normalized2 = pipeline._normalize_subtitles_for_quality(segments_prev_no_end)
    normalized3 = pipeline._normalize_subtitles_for_quality(segments_prev_end_none)
    
    assert len(normalized1) == 2
    assert len(normalized2) == 2
    assert len(normalized3) == 2


def test_normalize_subtitles_current_line_empty(tmp_path):
    """_normalize_subtitles_for_quality 内で current_line が空文字列の場合のブランチテスト"""
    pipeline = AntigravityPipeline(output_dir=tmp_path / "output")
    # text にスペースのみを入れることで、早期 return をバイパスしつつ、clean_text を空文字列にする
    segments = [
        {"id": "seg_001", "start": 0.0, "end": 1.0, "text": "   "}
    ]
    normalized = pipeline._normalize_subtitles_for_quality(segments)
