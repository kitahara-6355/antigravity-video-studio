...
logger = logging.getLogger("director_engine_archive")
...
"""
            ...
            nexus_logger.warning(f"Nexus Dispatch Error: {e}", exc_info=True)
            # フォールバック：全エージェント
            return {
                "intent": "解析失敗",
...
"""
            ...
            logger.error(f"Analyst Response API Error: {e}", exc_info=True)
            return f"分析エラー: {e}"

    def _get_system_instruction(self, mode="consult"):
        """
        Dynamically constructs the system instruction based on the active Brain (mode).
        """
        # Load latest context
        bm = branding_manager
        c = bm.constitution
        ...
        channel_name = c.get('channel_name', 'Channel')
        
        # Resolve Status Levels
        biz_rank = u.get('ranks', {}).get('biz_rank', {}).get('level', 'Novice')
        tech_rank = u.get('ranks', {}).get('tech_rank', {}).get('level', 'Novice')
        
        # Resolve Advice Mode based on Rank
        biz_advice = "初心者なので、専門用語を避け、具体的で簡単なアクションアイテム（Do This）を提示してください。" if biz_rank == 'Novice' else "プロ同士として、高度なマーケティング理論を用いた議論を行ってください。"
        tech_advice = "初心者なので、ツールの使い方は手取り足取り教え、複雑な演出は代わりにやってあげてください。" if tech_rank == 'Novice' else "上級者なので、抽象的なイメージ共有だけで意図を汲み取り、より洗練された演出を提案してください。"

        # Resolve Auto-Pilot Ratio
        auto_pilot_ratio = u.get('automation_settings', {}).get('auto_pilot_ratio', 0.9)
        ...
        # Select Persona
        if mode == "consult":
            base = self.persona_consultant.format(
                channel_name=channel_name,
                biz_rank=biz_rank,
                biz_advice_mode=biz_advice
            )
        else: # direct (director mode)
            base = self.persona_director.format(
                tech_rank=tech_rank,
        ...
        """
        # logger に注入された指示の一部を出力（デバッグ用）
        import logging
        d_logger = logging.getLogger("director_engine")
        d_logger.info(f"Generated System Instruction (len: {len(full_instruction)}). Deep Context preview: {deep_context[:100]}...")
    ...
    def consult(self, history, user_input):
        ...
        try:
            ...
            logger.error(f"Consult Error: {e}", exc_info=True)
    ...
    def generate_image(self, prompt, aspect_ratio="16:9"):
        ...
        try:
            ...
            logger.error(f"Image Gen Error Detail: {e}", exc_info=True)
            raise e
    ...
    def process_image_task(self, task_id, prompt):
        ...
        try:
            ...
            logger.error(f"Task {task_id} API Error: {e}", exc_info=True)
            task_manager.update_task(task_id, "failed", error=str(e))
        except Exception as e:
            logger.error(f"Task {task_id} Unexpected Failure: {e}", exc_info=True)
            task_manager.update_task(task_id, "failed", error=str(e))
    ...
    def analyze_script(self, full_text):
        ...
        try:
            ...
            logger.warning(f"Analysis Error: {e}", exc_info=True)
    ...
    def generate_storyboard_plan(self, full_text, scenes, selected_style):
        ...
        try:
            ...
            prompt = f"""
            You are the Art Director. Based on the selected style "{selected_style.get('name', '')}" ({selected_style.get('visual_prompt', '')}),
            create a detailed storyboard plan for the following scenes.
            
            ## Selected Style
            {selected_style.get('name', '')}: {selected_style.get('description', '')}
            ...
            logger.warning(f"Storyboard Plan Error: {e}", exc_info=True)
            # Fallback
            result = []
            for i, s in enumerate(scenes):
                result.append({
                    ...
                    "visual_prompt": f"Scene context: {s.get('name')}",
    ...
    def analyze_resource_needs(self, full_text):
        ...
        try:
            ...
            logger.warning(f"Resource Audit Error: {e}", exc_info=True)
            return json.dumps([])
    ...
    def calculate_quality_score(self, storyboard_plan, biz_rank="Novice"):
        ...
        try:
            ...
            logger.warning(f"Scoring Error: {e}", exc_info=True)
            return json.dumps({
    ...
    def process_batch_image_task(self, task_id, scenes, style_prompt):
        ...
        try:
            ...
            logger.error(f"Batch Task {task_id} API Error: {e}", exc_info=True)
            task_manager.update_task(task_id, "failed", error=str(e))
        except Exception as e:
            logger.error(f"Batch Task {task_id} Unexpected Failure: {e}", exc_info=True)
            task_manager.update_task(task_id, "failed", error=str(e))
    ...
    def verify_production_quality(self, full_text, scenes, segments):
        ...
        try:
            ...
            logger.warning(f"Quality Gate Error: {e}", exc_info=True)
            return json.dumps({
