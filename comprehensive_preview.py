...
def _parse_srt_subtitle_for_timestamp(srt_path: Path, timestamp: float) -> str:
    ...
    try:
        ...
        logger.warning(f"Failed to parse SRT for timestamp {timestamp}: {e}", exc_info=True)
    return ""
    ...
    try:
        with Image.open(path) as img:
            ...
            if padding_mode == "blur":
                ...
                bg_resized = img.resize((bg_w, bg_h), Image.Resampling.BILINEAR)
            ...
            try:
                ...
                try:
                    ...
                    mean_color_variance = sum(abs(x - y) for x, y in zip(r.getdata(), g.getdata())) / (new_w * new_h)
                    color_factor = 1.25 if mean_color_variance < 15.0 else 1.15
                except (ValueError, AttributeError, OSError, ZeroDivisionError):
                    color_factor = 1.15
                ...
                stat_img = resized_img.convert("L")
                mean_brightness = sum(stat_img.getdata()) / (new_w * new_h)
